...
class MCU_pseudoscaled_adc:
    def __init__(self, main, pin_params, pin_number):
        self._main = main
    ...
    def _handle_callback(self, read_time, read_value):
        self._main.pin_callback(self.pin_number, read_time, read_value)
...
class PrinterADCPseudoscaled:
    ...
    def setup_pin(self, pin_type, pin_params):
        ...
        pin_number = len(self.last_readings)
        self.last_readings.append(-1)
        return MCU_pseudoscaled_adc(self, pin_params, pin_number)
    def pin_callback(self, pin_number, read_time, read_value):
        self.last_readings[pin_number] = read_value
        count = 0
        sum = 0
        for i in self.last_readings:
            if i >= 0:
                count += 1
                sum += i
        avg = sum / count
        estimated_vref = 0.9860329515389469 + avg*0.010659926798205807  # bulharský polynom
        self.vref_callback(read_time, estimated_vref)
...
def load_config_prefix(config):
    return PrinterADCPseudoscaled(config)
